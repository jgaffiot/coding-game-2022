...
constexpr uint R_FOCUS{5000u};
constexpr uint R_CTRL{2200u};
constexpr uint MANA_COST{10u};
constexpr uint GOAL_REACH = 3 * WIND_MOV + R_BASE;
constexpr uint SETUP = static_cast<uint>(floor(sqrt(pow(GOAL_REACH, 2) / 2)));
...
const array<Point, 2> kSetup{
    Point{X_MIN + SETUP, Y_MIN + SETUP}, Point{X_MAX - SETUP, Y_MAX - SETUP}};

const array<array<Point, 2>, 2> kSpawn{
    {{{Point{X_MAX - 4000, Y_MIN + R_DMG}, Point{X_MAX - 7500, Y_MAX - R_DMG}}},
     {{Point{X_MIN + 4000, Y_MAX - R_DMG}, Point{X_MIN + 7500, Y_MIN + R_DMG}}}}};

const array<array<Point, 2>, 2> kFort{
    {{{Point{X_MAX - 4000, Y_MAX - 2000}, Point{X_MAX - 2000, Y_MAX - 4000}}},
     {{Point{X_MIN + 4000, Y_MIN + 2000}, Point{X_MIN + 2000, Y_MIN + 4000}}}}};

const array<array<Point, 2>, 2> kSneaky{
    {{{Point{X_MIN + R_FOCUS + MSTR_MOV, Y_MIN + MSTR_MOV},
       Point{X_MIN + MSTR_MOV, Y_MIN + R_FOCUS + MSTR_MOV}}},
     {{Point{X_MAX - R_FOCUS - MSTR_MOV, Y_MAX - MSTR_MOV},
       Point{X_MAX - MSTR_MOV, Y_MAX - R_FOCUS - MSTR_MOV}}}}};

const array<Point, 2> kOutpost{
    Point{X_MIN + (WIND_MOV + 0), Y_MIN + (WIND_MOV + 0)},
    Point{X_MAX - (WIND_MOV + 0), Y_MAX - (WIND_MOV + 0)}};
...
public:
    ...
    // Point would_land(Point target, int dist) {
    //     Vector2 speed = Vector2::from_polar(dist, (target - p()).theta());
    //     Point res = p();
    //     res.advance(speed);
    //     return res;
    // }

    // int turn_to_catch_up(const Point& p) {
    //     Point m = p_;
    //     Point h = p;
    //     int res = 0;
    //     while(dist(m, h) > R_DMG) {
    //         m = would_land(Home(), MSTR_MOV);
    //         h = would_land(m, HERO_MOV);
    //         res++;
    //     }
    //     return res;
    // }
    ...
    int turn_to_kill() {
        ...
        return static_cast<int>(
            ceil((p().dist_to(threat_destination(threat_)) - R_BASE) / MSTR_MOV));
    }

    bool soloable(const Point& p) {
        return (turn_to_kill() - dist_to(p) / (HERO_MOV - MSTR_MOV)) * DMG >= health_;
    }
...
public:
    ...
    Point scout() {
        ...
        if (g_turn < 50) {
            return kSpawn[top_left][id() % 2];
        } else {
            return kFort[top_left][id() % 2];
        }
    }
...
public:
    ...
    uint mana;  // Spend ten mana to cast a spell
    map<int, Hero> heroes;
    ...
    // dirty use of get_closest for other
    map<int, Monster> deque_to_map(deque<int> q) {
        map<int, Monster> res;
        for (int id : q) {
            res.insert({id, monsters.at(id)});
        }
        return res;
    }
        ...
        for (uint i = 0; i < NB_HEROES; i++) {
            if (i == 0) {
                ...
                if (monsters.empty()) {
                    ...
                    debug("// Farm");
                    if (auto [dist, id] = get_closest(monsters, h);
                        id != -1 and dist < 2 * HERO_MOV) {
                        h.order(Action::kMove, monsters.at(id).p());
                        // monsters.erase(id);
                    } else {
                    ...
                    debug("// Deal with danger");
                    Monster& m = monsters.at(dangers.front());
                    auto [vdist, vid] = get_closest(vilains, Home());
                    if (m.dist_to(Home()) <= R_BASE + MSTR_MOV and not m.shield()
                        and h.dist_to(m.p()) < R_WIND)
                    {
                        debug("// Panic wind");
                        h.order(Action::kWind, Opponent());
                        monsters.erase(m.id());
                        ...
                        and debug(
                            "vilain dist: ",
                            vilains.at(vid).dist_to(Home()),
                            " <? ",
                            WIND_MOV + R_WIND)
                        and m.dist_to(Home()) <= R_BASE + WIND_MOV
                        and vilains.at(vid).dist_to(m.p()) <= R_WIND
                        and h.dist_to(m.p()) <= R_WIND and not m.shield()
                        and mana >= MANA_COST)
                    {
                        debug("// Avoid wind goal attack");
                        h.order(Action::kWind, Opponent());
                        monsters.erase(m.id());
                        dangers.pop_front();
                        // } else if (m.health() > 14 and not m.shield() and
                        // h.dist_to(m.p()) < R_CTRL and mana >= 3 * MANA_COST) {
                        //     debug("// Sneaky ctrl");
                        //     h.order(Action::kControl, kSneaky[top_left][i%2],
                        //     m.id()); monsters.erase(m.id()); dangers.pop_front();
                    } else {
                        debug("// Default auto attack");
                        if (i == 1 && heroes.at(2).dist_to(m.p()) < h.dist_to(m.p())
                            and m.soloable(heroes.at(2).p()))
                        {
                            // Hero 2 is closer, he will deal with it
                            if (dangers.size() > 1) {
                                debug("// Danger 2");
                                h.order(Action::kMove, monsters.at(dangers.at(1)).p());
                            } else {
                                debug("// WHAT TO DO Farm");
                                if (auto [dist, id] = get_closest(monsters, h);
                                    id != -1 and dist < 2 * HERO_MOV) {
                                    h.order(Action::kMove, monsters.at(id).p());
                                    // monsters.erase(id);
